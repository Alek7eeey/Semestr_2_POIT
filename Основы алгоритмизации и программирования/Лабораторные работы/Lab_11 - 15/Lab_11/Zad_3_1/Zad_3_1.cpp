...
int check_left(Tree* tree) //количества вершин дерева, являющихся левыми дочерними вершинами
{
	static int sum = 0;
	if (tree->Left != NULL )
	{
...
void main()
{
	...
	for (;;)
	{
		...
		case 9: { static int k = 0;
			int g = check_left(Root);
			cout << "Количество левых дочерних элементов = " << g-k << endl;
			k = g;
			break;
		}
		case 10: {
